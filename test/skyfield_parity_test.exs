defmodule Sgp4Ex.SkyfieldParityTest do
  @moduledoc """
  Bit-exact (0 ULP) parity test against Skyfield's TEME → GCRS transformation.

  Verifies that our native C++ implementation produces the exact same IEEE 754
  doubles as Skyfield for the reference case. Uses full-precision hex-captured
  reference values from Skyfield's python-skyfield checkout.
  ...
  # ISS TEME state at 2018-07-04 00:00:00 UTC
  @teme_state %{
  ...
  # Authoritative Skyfield reference captured via float.hex() from the local
  # python-skyfield checkout (Skyfield's _at() pipeline: km → AU → rotate → km).
  #
  ...
  test "direct reference case matches Skyfield at 0 ULP" do
    result = Sgp4Ex.teme_to_gcrs(@teme_state, {{2018, 7, 4}, {0, 0, 0}})

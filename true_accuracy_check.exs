...
IO.puts("  Difference:    #{Float.round(gast_diff, 18)} hours")
...
# Accuracy assessment
if dpsi_microarcsec < 1.0 and deps_microarcsec < 1.0 and gast_millisec < 0.001 do
  IO.puts("\n🎉 EXCELLENT ACCURACY - Sub-microarcsecond precision!")
elsif dpsi_microarcsec < 100.0 and deps_microarcsec < 100.0 and gast_millisec < 0.1 do
  IO.puts("\n✅ VERY GOOD ACCURACY - Within acceptable limits")
elsif dpsi_microarcsec < 1000.0 and deps_microarcsec < 1000.0 and gast_millisec < 1.0 do
...
IO.puts("  Accuracy: #{Float.round(dpsi_microarcsec, 1)}μas nutation error")
